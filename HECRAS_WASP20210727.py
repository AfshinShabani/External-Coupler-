...
try:
    ...
    with open(os.path.join(r'',Config_File,'Configuration.txt'),'r+') as address:  
        ...
        address.close()
    PATH_HECRAS=os.path.join(r'',lines[14].rstrip())  # PATH to HEC-RAS 2D Grid shapefile 
    PATH_WASP=os.path.join(r'',lines[15].rstrip())    # PATh to WASP Grid shapefile 
    HDF=os.path.join(r'',lines[16].rstrip())          # PATH to HEC-RAS Output
    PostProcess=os.path.join(r'',lines[17].rstrip())  # PATH to HEC-RAS Postprocess File
    PATH=os.path.join(r'',lines[18].rstrip())         # PATH to output
    ...
    if not os.path.exists(PATH):
       try:
           #os.mkdir(os.path.join(os.path.dirname(os.path.abspath(HDF)),'Output'))
           os.mkdir(os.path.join(Config_File,'Output'))
           #PATH=os.path.dirname(os.path.abspath(HDF))+'\Output'
    ...
    def SEGMENT_VOLUME():
        ...
        for index,name in MD.iterrows(): # Put number of segments here
            ...
            if SegmentVolume[index,0]<MinVolume:    
                ...
                FlowD5['0']=SegmentVolume[index,0] 
    ...
    def terminate ():
         if len(Segment)>0:
             ...
             print ('Error: To resolve volume error, additional volume for wet/dry segmets shoule be set to %s' %(np.min(minvl)*-1))
             time.sleep(10)#^10000)
             sys.exis()
    maxVolume=np.abs(np.min(minvl))+1
    SegVolume,Segment,minvl,FlowD=SEGMENT_VOLUME()         

    # if len(Segment)>0:  # add this part to the code remove it please 
    #     terminate() ####
        
    if len (HECRAS_CELL)> len (MD):
        Itr=1
        while len(Segment)>0 and Itr<10:
            if Itr==1:
    ...
    with open (os.path.join(PATH,'External Coupler Log.txt'),'w+') as logfile:
        logfile.write('Date and time :%s' %str(datetime.datetime.now())+'\n')
        logfile.write('External Coupler runtime: %s (seconds)'%round(end-Start,0)+'\n')
        logfile.write('Number of HEC-RAS 2D cells: %s'%len(HECRAS_CELL)+'\n')
        logfile.write('Number of WASP segments: %s'%len(SegVolume)+'\n')
        logfile.write('Number of flow for WASP segment: %s'%NFLOW+'\n')
        logfile.write('Total time steps for hydrodynamic data: %s'%Dcolumns+'\n')
        logfile.write('Hydrolink file: %s2D.hyd'%Hydro_name+'\n')
        if Num_layer >1:
           logfile.write('Sediment layer is included'+'\n')
           logfile.write('Depth of sediment layer: %s '%Sediment_depth+'\n')
    logfile.close()
    
    input ("The WASP Hydrodynamic input file was sucessfully generated and data are stored at "
    "%s"
    " please press Enter to close this window and see External Coupler Log.txt for more infomration"%PATH)
